...
use common::{
    crypto::{create_signature, load_private_key, KeyType, PrivateKey},
    param::PASSWORD,
...
use tss2_fapi_rs::{
    ActnCallback, ActnCallbackParam, BranCallback, BranCallbackParam, FapiContext, KeyFlags,
    SignCallback, SignCallbackParam,
};
...
fn test_policy_signed_rsa() {
    ...
    repeat_test!(|i| {
        ...
        let policy_json = read_policy(configuration.data_path(), "pol_signed_rsa")
            .expect("Failed to read policy!");

        // Read the private key
        let private_key =
            read_private_key(configuration.data_path(), KeyType::RsaKey, "key_rsa_2048")
...
fn test_policy_signed_ecc() {
    ...
    repeat_test!(|i| {
        ...
        // Read policy from file
        let policy_json = read_policy(configuration.data_path(), "pol_signed_ecc")
            .expect("Failed to read policy!");

        // Read the private key
        let private_key =
            read_private_key(configuration.data_path(), KeyType::EccKey, "key_ecc_256")
                .expect("Failed to load ECC private key!");
...
fn test_policy_or() {
    ...
    repeat_test!(|i| {
        ...
        // Read policy from file
        let policy_json =
            read_policy(configuration.data_path(), "pol_or").expect("Failed to read policy!");
        ...
        match context
            .set_branch_callback(BranCallback::with_data(my_bran_callback, branches.clone()))
        {
        ...
        let branches_ref = branches
            .try_lock()
            .expect("Failed to borrow the result value!");
...
fn test_policy_action() {
    ...
    repeat_test!(|i| {
        ...
        // Read policy from file
        let policy_json =
            read_policy(configuration.data_path(), "pol_action").expect("Failed to read policy!");

        // Set up callback
        let actions: Arc<Mutex<Vec<String>>> = Arc::new(Mutex::new(Vec::new()));
        match context
            .set_policy_action_callback(ActnCallback::with_data(my_actn_callback, actions.clone()))
        {
...
fn read_policy(data_path: &Path, policy_name: &str) -> Option<JsonValue> {
    let policy_file = data_path
        .join("policies")
        .join(format!("{}.json", policy_name));
    fs::read_to_string(policy_file)
        .ok()
        .and_then(|policy_data| json::parse(&policy_data[..]).ok())
}
...
fn read_private_key(data_path: &Path, key_type: KeyType, key_name: &str) -> Option<PrivateKey> {
    ...
    fs::read_to_string(pem_file)
        .ok()
        .and_then(|pem_data| load_private_key(&pem_data[..], key_type))
}
...
fn my_sign_callback(param: SignCallbackParam, private_key: &PrivateKey) -> Option<Vec<u8>> {
    debug!(
        "(SIGN_CB) Signature for {:?} has been requested!",
        param.object_path
    );
    ...
    if let Some(signature_bytes) = signature_value.as_ref() {
        debug!(
            "(SIGN_CB) Computed signature value: {}",
...
fn my_bran_callback(param: BranCallbackParam, branches: &Arc<Mutex<Vec<String>>>) -> Option<usize> {
    debug!(
        "(BRAN_CB) Branch for {:?} has been requested!",
        param.object_path
    );
